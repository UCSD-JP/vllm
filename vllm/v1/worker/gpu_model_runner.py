...
class GPUModelRunner(
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        Returns:
            Model output tensor
        """
        # Expert cache v1 step() removed — now handled by pre_step() in
        # execute_model() before CUDA graph replay.
    ...
    def execute_model(
        ...
        # Expert cache v2: predict-and-preload before CUDA graph replay.
        # pre_step() reads previous routing, fetches misses, updates _cache_map.
        if hasattr(self, '_expert_cache') and self._expert_cache is not None:
            if hasattr(self, '_expert_cache_layers'):
                # HIGH-2: ubatch guard — PoC does not support ubatching
                # (single routing_snapshot buffer would be overwritten)
                if should_ubatch:
                    cudagraph_mode = CUDAGraphMode.NONE
                    logger.warning(
                        "Expert offload v2: ubatch active, "
                        "falling back to eager (not supported in PoC)")
                else:
                    # HIGH-1: pass unpadded num_tokens to trim padding
                    cache_result = self._expert_cache.pre_step(
                        self._expert_cache_layers,
                        num_tokens=num_tokens_unpadded)
                    # MEDIUM-1: miss_ratio fallback (proportional to batch)
                    # With predict-and-preload, cache misses cause
                    # zero-weighted expert contribution for 1 step —
                    # the model self-corrects next step. Eager fallback
                    # is ~20x slower, so only use it for catastrophic
                    # miss rates (default 0.95). Set to 1.0 to disable.
                    miss_ratio_threshold = getattr(
                        self, '_expert_miss_ratio_threshold', 0.95)
                    ...
                    if (step > warmup_steps
                        ...
                        cudagraph_mode = CUDAGraphMode.NONE
                        logger.warning(
                            "Expert cache miss_ratio=%.2f > threshold=%.2f "
                            "(misses=%d, routed=%d, step=%d), eager fallback",
                            cache_result['miss_ratio'],
                            miss_ratio_threshold,
                            cache_result['total_misses'],
                            cache_result['total_routed'],
                            step)
                    elif cache_result['miss_ratio'] > 0.10:
                        # Log high miss ratio without fallback
                        if step % 100 == 0:  # throttle logging
                            logger.info(
                                "Expert cache miss_ratio=%.2f "
                                "(misses=%d, routed=%d, step=%d)",
                                cache_result['miss_ratio'],
                                cache_result['total_misses'],
                                cache_result['total_routed'],
                                step)
                    ...
                    if step > 0 and step % 100 == 0:
                        ...
                        logger.info("Step %d: %s", step, summary)
