...
class FusedMoE(CustomOp):
    ...
    def set_expert_cache(self, cache, predictor, layer_idx,
        ...
        v2: Registers persistent buffers for CUDA graph compatibility.
        - _cache_map: slot mapping read by kernel during graph replay
        - _routing_snapshot: captures topk_ids for next-step prediction
        - _routing_len: valid length of routing_snapshot
        """
        self._expert_cache = cache
        self._expert_predictor = predictor
        self._layer_idx = layer_idx

        # v2: Register persistent buffers (data_ptr fixed for CUDA graph)
        if max_num_tokens is not None and max_num_tokens > 0:
            # _cache_map[gid] = slot_idx or -1; read by kernel in graph
            self.register_buffer(
                '_cache_map',
                torch.full((self.global_num_experts,), -1,
                           dtype=torch.int32,
                           device=self.w13_weight.device),
                persistent=False,
            )
            # _routing_snapshot: captures topk_ids.flatten() during graph
            top_k = getattr(self, 'top_k', 10)
            self.register_buffer(
                '_routing_snapshot',
                torch.zeros(max_num_tokens * top_k,
                            dtype=torch.int32,
                            device=self.w13_weight.device),
                persistent=False,
            )
            # _routing_len: valid length written by .fill_(n) in graph
            self.register_buffer(
                '_routing_len',
    ...
    def update_expert_map(self):
        ...
        # Re-cache expert_map in offload cache (prevents stale mapping after EPLB)
        if self._expert_cache is not None and self._layer_idx >= 0:
            if hasattr(self._expert_cache, 'invalidate_emap_cache'):
                self._expert_cache.invalidate_emap_cache(
                    self._layer_idx, new_expert_map=self._expert_map)
