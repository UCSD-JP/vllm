...
class UnquantizedFusedMoEMethod(FusedMoEMethodBase, CustomOp):
    ...
    def forward_cuda(
        ...
        # Expert cache integration (v2: CUDA graph compatible)
        # _cache_map is a persistent buffer updated by pre_step() before graph
        # _routing_snapshot captures current topk_ids for next step's pre_step()
        # All ops here are GPU-only (.copy_(), .fill_()) → graph-safe
        expert_map = layer.expert_map
        if hasattr(layer, '_cache_map') and layer._cache_map is not None:
            expert_map = layer._cache_map  # persistent buffer, fixed data_ptr

            # Capture routing for next step (GPU→GPU copy, graph-safe)
            if hasattr(layer, '_routing_snapshot') \
                ...
                layer._routing_snapshot[:n].copy_(flat[:n])
                layer._routing_len.fill_(n)
